...
def adjust_outlier_probability(X, y, p, num_samples_total=None, verbosity=0):
    ...
    if num_samples_total is not None:
        ...
        if num_normal_samples > len(X_normal):
            warnings.warn(
                f"Only {len(X_normal)} normal samples available,"
            ...
            raise ValueError(
                f"Not enough normal samples available to adjust"
    ...
    if verbosity > 0:
        print(
            f"Test samples: {len(X)} -- Outlier probability: {len(X_anomaly) / len(X)*100:.2f}%"
        )
...
def analyze_neighbor_distances(
    X, noise_std=None, plot=False, percentile=90, n_neighbors=4
):
...
def sionna_anomaly_detection(
    ...
    meas_filename = os.path.join(
        module_path, "datasets", f"scene{scene_nr}_measurements{dataset_nr}.pkl"
    )
    ...
    for k, (train_idx, test_idx) in enumerate(kf.split(measurements_scaled, labels)):
        ...
        jammer_test = np.array(measurement_collection.jammers_list, dtype=object)[
            test_idx
        ]
        ...
        if method == "lof":
            model = unsupervised_detection.LocalOutlierFactorLearning(
                n_neighbors=cfg.lof.n_neighbors, probability=probability
            )
        elif method == "energy_detector":
            model = unsupervised_detection.EnergyDetector(
                percentile=0.95, probability=probability
            )
        else:
        ...
        X_test, y_test = adjust_outlier_probability(
            X_test,
            y_test,
        ...
        if probability:
            results_filenname = os.path.join(
                module_path, "datasets", "results", f"sionna_{dataset_nr}_prob.pkl"
            )
        else:
            results_filenname = os.path.join(
                module_path, "datasets", "results", f"sionna_{dataset_nr}.pkl"
            )
        ...
        if k == 0:
            ...
            results[method][noise_std]["y_test"] = np.concatenate(
                (results[method][noise_std]["y_test"], y_test)
            )
            results[method][noise_std]["y_hat"] = np.concatenate(
                (results[method][noise_std]["y_hat"], y_hat)
            )
            results[method][noise_std]["jammer"] = np.concatenate(
                (results[method][noise_std]["jammer"], jammer_test)
            )
