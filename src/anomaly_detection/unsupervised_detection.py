...
class EnergyDetector:
    ...
    def __init__(
        ...
        if statistic in ["mean", "median", "skewness", "var"]:
            ...
            raise NotImplementedError(
                f"Statistic {statistic} is not implemented.^"
    ...
    def visualize_distributions(self, X_train, X_test, y_train, y_test):
        ...
        if np.isnan(self.threshold):
            warnings.warn(
                "The classifier has not yet been fitted. Hence, no threshold is shown."
            )
        else:
...
class LocalOutlierFactorLearning:
    ...
    def __init__(
        self, n_neighbors=20, algorithm="auto", contamination="auto", probability=False
    ):
        """Initialization
