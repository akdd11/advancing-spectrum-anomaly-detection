...
range_num_tx = [cfg.min_num_tx, cfg.max_num_tx]     # minimum and maximum number of transmitters
...
range_num_jam = [cfg.min_num_jam, cfg.max_num_jam]      # minimum and maximum number of jammers
...
for i in trange(cfg.num_radiomaps):
    radio_map = RadioMap(pl_results.config['scene_size'][:2],
    ...
    for i_tx in range(num_tx):
        ...
        radio_map.add_transmitter('tx', pl_results.pathlossmaps[idx].tx_pos,
                                  tx_power, pl_results.pathlossmaps[idx].pathloss)
    ...
    for i_jam in range(num_jam):
        ...
        radio_map.add_transmitter('jammer', pl_results.pathlossmaps[idx].tx_pos,
                                  jam_power, pl_results.pathlossmaps[idx].pathloss)
