...
from src.utils.pl_utils import (
    PathLossMap,
    PathLossMapCollection,
)  # required to load pathloss map results from pickle file
from src.utils.pl_utils import (
    enclosed_in_obstacle,
    create_sionna_sample,
    get_obstacle_mask,
)
...
# Plot configuration
latex_installed = (
    True if find_executable("latex") else False
...
if cfg.dt_generation not in ["accurate", "ml"]:
    raise ValueError(
        'Invalid value for dt_generation. Only "accurate" and "ml" are allowed.'
    )
...
rm_dataset_nr = (
    cfg.rm_dataset_nr
)  # number of the path loss and radio map dataset to load
...
if not cfg.use_original_pl_maps:
    scene_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "scenes", f"scene{scene_nr}", "scene.xml"
    )
...
measurements_filename = (
    f"scene{scene_nr}_measurements{meas_dataset_nr}.pkl"  # measurements dataset (save)
)
...
if cfg.save_radio_maps:
    ...
    if cfg.dt_generation == "ml":
        warnings.warn(
            "No radiomaps created for dt_generation=ml. No plots will be saved.",
            UserWarning,
        )
...
# If sionna, extract and apply further configurations
# from path loss map collection
...
# Collect errors between digital twin (without jammer) and the
# radio map measured (eventually containing jammer)

jammed_diffs = {"mean": [], "median": []}
not_jammed_diffs = {"mean": [], "median": []}

# Define the measurement points
meas_x, meas_y = radiomap_utils.generate_measurement_points(
    measurement_method, radiomaps[0].radio_map.shape
)

obstacle_mask = get_obstacle_mask(
    scene_nr, scene.size.numpy().astype(int), config["rx_height"]
)
...
if cfg.dt_generation == "ml":
    ...
    model, scaler_x, scaler_y = train_model(
        df, validate=True, use_dist=cfg.ml_use_distances, num_sus=len(meas_x)
    )
    end = time.time()
...
measurement_collection = radiomap_utils.MeasurementCollection(
    measurement_method, meas_x, meas_y
)
...
for rm_idx, rm_orig in enumerate(tqdm(radiomaps)):
    ...
    if cfg.use_original_pl_maps:
        ...
        if cfg.dt_generation == "accurate":
            ...
            rm_dt = radiomap_utils.RadioMap(
                rm_orig.radio_map.shape, resolution, rm_orig.noise_floor
            )
            for tx in rm_orig.transmitters:
                # Recreate path loss map for estimated transmitter position
                tx_pos_est = add_localization_error()
                pathlossmap = create_sionna_sample(
                    config, scene, scene_nr, tx_pos=tx_pos_est
                ).pathloss
            ...
            if cfg.plot_radio_maps or cfg.save_radio_maps:
                ...
                v_min = np.nanmin(
                    [np.nanmin(rm_orig_obstacles), np.nanmin(rm_dt_obstacles)]
                )
                v_max = np.nanmax(
                    [np.nanmax(rm_orig_obstacles), np.nanmax(rm_dt_obstacles)]
                )
                if cfg.save_radio_maps:
                    filename_orig = os.path.join(
                        figure_path, f"radiomap_orig_{rm_idx}.png"
                    )
                    filename_dt = os.path.join(figure_path, f"radiomap_dt_{rm_idx}.png")
                    filename_diff = os.path.join(
                        figure_path, f"radiomap_diff_{rm_idx}.png"
                    )
                else:
                    filename_orig = None
                    filename_dt = None
                ...
                # plot the difference
                radiomap_utils.plot_radio_map_difference(
                    rm_orig,
                    rm_dt,
                    plot_orig_tx=False,
                    plot_dt_tx=False,
            ...
            measurements_dt = np.zeros(len(meas_x)).reshape(
                1, -1
            )  # RSS, for the beginning in linear scale

            for tx in rm_orig.transmitters:
                tx_pos_est = add_localization_error()

                tx_input = prepare_input(
                    tx_pos_est,
                    meas_x,
                    meas_y,
                    scaler_x,
    ...
    measurement_collection.add_measurement(
        rm_orig.transmitters, rm_orig.jammers, measurements_orig, measurements_dt
    )
...
shutil.copyfile(
    os.path.join(dataset_dir, rm_filename[:-4] + ".txt"),
