...
def generate_df(
    ...
    if use_dist:
        ...
        col_names = (
            ["tx_x", "tx_y"]
            + [f"dist_{i}" for i in range(len(meas_x))]
            + [f"su_{i}" for i in range(len(meas_x))]
        )
    else:
        save_filename = f"scene{scene_nr}_pl_for_ML_{dataset_nr}.csv"
        col_names = ["tx_x", "tx_y"] + [f"su_{i}" for i in range(len(meas_x))]
    save_path = os.path.join(
        module_path, "datasets", "supervised learning", save_filename
    )
    ...
    for sample_id in tqdm(range(len(plmc.pathlossmaps))):
        ...
        pathloss_values = [
            plmc.pathlossmaps[sample_id].pathloss[int(meas_x[i]), int(meas_y[i])]
            for i in range(len(meas_x))
        ]
        if use_dist:
            dist_list = [
                distance.euclidean(plm_df, [k, l]) for k, l in zip(meas_x, meas_y)
            ]
...
def train_model(df, validate=False, algo="mor", use_dist=True, num_sus=None):
    ...
    if use_dist:
        if num_sus is None:
            raise ValueError(
                "Number of SUs must be specified, if the distances shall be used as input feature."
            )
    ...
    if algo == "mor":
        ...
        model = MultiOutputRegressor(
            RandomForestRegressor(n_estimators=400, min_samples_split=2, n_jobs=None),
            n_jobs=None,
        )
...
def prepare_input(
    tx_pos_est, meas_x, meas_y, scaler_x, measurement_method, use_dist=True
):
    ...
    if use_dist:
        ...
        if measurement_method == "custom1":
            dist_list = [
                distance.euclidean(tx_pos_est[0:2], [k, l])
                for k, l in zip(meas_x, meas_y)
            ]
        else:
