...
class MeasurementCollection:
    ...
    def add_measurement(
        self, transmitters, jammers, measurements_orig, measurements_dt
    ):
...
class RadioMap:
    ...
    def add_transmitter(self, tx_type, tx_pos, tx_power, pathloss_map):
        ...
        if np.any(pathloss_map < 0):
            raise ValueError(
                "Pathloss map contains the loss, i.e., values must be > 0."
            )
        single_radio_map = (
            tx_power - pathloss_map
        )  # note: pathloss_map values shall be > 0
        ...
        if (len(self.transmitters) + len(self.jammers)) == 1:
            ...
            self.radio_map = 10 * np.log10(
                10 ** (self.radio_map / 10) + 10 ** (single_radio_map / 10)
            )

    def show_radio_map(
        self, plot_transmitters=True, rm_type="orig", show_plot=True, **kwargs
    ):
        ...
        if rm_type == "orig":
            cbar_label = (
                "$P_\mathrm{rx} [\mathrm{dBm}]$" if latex_installed else "P_rx [dBm]"
            )
        elif rm_type == "dt":
            cbar_label = (
                r"$\hat{P}_\mathrm{rx} [\mathrm{dBm}]$"
                if latex_installed
                else "P^hat_rx [dBm]"
            )
        ...
        if plot_transmitters:
            ...
            for transmitter in self.transmitters:
                if rm_type == "orig":
                    plt.plot(
                        transmitter.tx_pos[0] + self.plot_offset,
                    ...
                    plt.plot(
                        ...
                        transmitter.tx_pos[1] + self.plot_offset,
                        "v",
                        markerfacecolor=COLOR_TX_DT_FACE,
            ...
            for jammer in self.jammers:
                ...
                plt.plot(
                    jammer.tx_pos[0] + self.plot_offset,
                    jammer.tx_pos[1] + self.plot_offset,
                    "X",
                    markerfacecolor=COLOR_JAMMER_FACE,
        ...
        if rm_type == "orig":
            plt.plot(
                [], [], "v", color=COLOR_TX_ORIG, markersize=10, label="Transmitter"
            )
        else:
            plt.plot(
                [],
...
def generate_measurement_points(method, shape, **kwargs):
    ...
    if method == "grid":
        ...
        def calc_offset(length, grid_size):
            ...
            return np.floor(
                (length - (np.floor(length / grid_size) * grid_size)) / 2
            ).astype(int)
...
def plot_radio_map_difference(
    ...
    cbar_label = (
        r"$P_\mathrm{rx} - \hat{P}_\mathrm{rx} [\mathrm{dB}]$"
        if latex_installed
        else "P_rx - P^hat_rx [dB]"
    )
    ...
    for transmitter in rm_orig.transmitters:
        plt.plot(
            transmitter.tx_pos[0] + res_offset,
            transmitter.tx_pos[1] + res_offset,
            "v",
            color=COLOR_TX_ORIG,
            markersize=10,
        )
    for jammer in rm_orig.jammers:
        plt.plot(
            jammer.tx_pos[0] + res_offset,
            jammer.tx_pos[1] + res_offset,
            "X",
            markerfacecolor=COLOR_JAMMER_FACE,
            markeredgecolor=COLOR_JAMMER_EDGE,
            markersize=10,
    ...
    if len(meas_x) > 0:
        plt.scatter(
            np.array(meas_x) + res_offset,
            np.array(meas_y) + res_offset,
            marker=".",
            color="black",
            label="Sensing unit (SU)",
        )

    # Create legend entries
    if plot_orig_tx:
        plt.plot(
            [], [], "v", color=COLOR_TX_ORIG, markersize=10, label="True transmitter"
        )
...
def reshape_measurement_collection_to_2D(meas_x, meas_y, measurements1D):
    ...
    for i in range(len(measurements1D)):
        measurements2D.append(
            reshape_measurements_to_2D(
                meas_x, meas_y, shape, grid_size, measurements1D[i]
            )
        )
