...
class PathLossMap:
    ...
    def show_pathloss_map(self, show_tx_pos=True):
        ...
        vmax = np.max(
            self.pathloss[np.isfinite(self.pathloss)]
        )  # set biggest value that does not equal inf as vmax
...
def create_sionna_sample(config, scene, scene_nr, tx_pos=None, num_rays=int(1e6)):
    ...
    cm = scene.coverage_map(
        ...
        cm_center=tf.cast(
            [scene_size[0] / 2, scene_size[1] / 2, config["rx_height"]],
            dtype=tf.float32,
        ),
        ...
        cm_cell_size=(
            config["cell_size"],
            config["cell_size"],
        ),  # Grid size of coverage map cells in m
    ...
    path_loss_db = -10 * np.log10(
        cm.as_tensor().numpy()[0].T
    )  # minus to convert gain to loss, transpose as otherwise x and y are swapped
