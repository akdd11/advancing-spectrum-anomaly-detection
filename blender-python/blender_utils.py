...
def init_materials(texture_path, initialize_textures=False):
    ...
    if initialize_textures:
        ...
        texture_nodes_concrete = {
            k: nodes_concrete.new(type="ShaderNodeTexImage")
            for k in ["color", "metalness", "roughness"]
        }

        # Set the image path
        texture_path_concrete = os.path.join(texture_path, "ConcretePoured")
        texture_nodes_concrete["color"].image = bpy.data.images.load(
            os.path.join(
                texture_path_concrete, "ConcretePoured001_COL_2K_METALNESS.png"
            )
        )
        texture_nodes_concrete["metalness"].image = bpy.data.images.load(
            os.path.join(
                texture_path_concrete, "ConcretePoured001_METALNESS_2K_METALNESS.png"
            )
        )
        texture_nodes_concrete["roughness"].image = bpy.data.images.load(
            os.path.join(
                texture_path_concrete, "ConcretePoured001_ROUGHNESS_2K_METALNESS.png"
            )
        )

        # Link nodes: texture color to Principled BSDF base color
        materials["concrete"].node_tree.links.new(
            texture_nodes_concrete["color"].outputs["Color"],
            bsdf_shader_concrete.inputs["Base Color"],
        )
        materials["concrete"].node_tree.links.new(
            texture_nodes_concrete["metalness"].outputs["Color"],
            bsdf_shader_concrete.inputs["Metallic"],
        )
        materials["concrete"].node_tree.links.new(
            texture_nodes_concrete["roughness"].outputs["Color"],
            bsdf_shader_concrete.inputs["Roughness"],
        )

        # Add Material Output node
        material_output_concrete = materials["concrete"].node_tree.nodes.new(
            type="ShaderNodeOutputMaterial"
        )

        # Link BSDF output to Surface input
        materials["concrete"].node_tree.links.new(
            bsdf_shader_concrete.outputs["BSDF"],
            material_output_concrete.inputs["Surface"],
        )
        ...
        texture_nodes_metal = {
            k: nodes_metal.new(type="ShaderNodeTexImage")
            for k in ["color", "metalness", "roughness"]
        }

        # Set the image path
        texture_path_metal = os.path.join(texture_path, "MetalGalvanizedSteelWorn")
        texture_nodes_metal["color"].image = bpy.data.images.load(
            os.path.join(
                texture_path_metal, "MetalGalvanizedSteelWorn001_COL_2K_METALNESS.jpg"
            )
        )
        texture_nodes_metal["metalness"].image = bpy.data.images.load(
            os.path.join(
                texture_path_metal,
                "MetalGalvanizedSteelWorn001_METALNESS_2K_METALNESS.jpg",
            )
        )
        texture_nodes_metal["roughness"].image = bpy.data.images.load(
            os.path.join(
                texture_path_metal,
        ...
        # Link nodes: texture color to Principled BSDF base color
        materials["metal"].node_tree.links.new(
            texture_nodes_metal["color"].outputs["Color"],
            bsdf_shader_metal.inputs["Base Color"],
        )
        materials["metal"].node_tree.links.new(
            texture_nodes_metal["metalness"].outputs["Color"],
            bsdf_shader_metal.inputs["Metallic"],
        )
        materials["metal"].node_tree.links.new(
            texture_nodes_metal["roughness"].outputs["Color"],
            bsdf_shader_metal.inputs["Roughness"],
        )

        # Add Material Output node
        material_output_metal = materials["metal"].node_tree.nodes.new(
            type="ShaderNodeOutputMaterial"
        )

        # Link BSDF output to Surface input
        materials["metal"].node_tree.links.new(
            bsdf_shader_metal.outputs["BSDF"], material_output_metal.inputs["Surface"]
        )
